...
func handleStatus(ctx context.Context, rc *runner.RunContext) error {
	...
	if checkStatusState == "pending" {
		return runStatus(ctx, rc)
	}

	if checkStatusState == "running" {
		if !rc.DeployerDisabled("status") {
			return checkDeployerJob(ctx, rc, "status")
		}
...
func runStatus(ctx context.Context, rc *runner.RunContext) error {
	...
	if !rc.DeployerDisabled("status") {
		...
		if rc.SandboxAPIInUse() {
			result, err := sandboxGet(ctx, rc, "status")
			if err != nil {
		...
		if err := launchTowerJob(ctx, rc, "status", "", extraSpecVars, dynamicJobVars); err != nil {
